...
namespace IQMetrix.Beertap.IntegrationTests
{
    public class ContainerTests
    {
        ...
        class WindsorContainerCustomization : ICustomization
        {
            public void Customize(IFixture fixture)
            {
                fixture.Register(() =>
                {
                    ...
                    BootStrapper.Initialize(config, typeof(OfficeSpec).Assembly, typeof(OfficeSpec).Assembly, new DefaultApiContainer(config, windsorContainer, typeof(OfficeSpec).Assembly, typeof(OfficeSpec).Assembly), initializeHelpPage: false);
        ...
        public void ItShouldResolveEntryPointControllerSetInConfiguration()
        {
            ...
            // arrange
            var config = new HttpConfiguration();
            var sut = new DefaultApiContainer(config, new WindsorContainer(), typeof(OfficeSpec).Assembly, typeof(OfficeSpec).Assembly);
            BootStrapper.Initialize(config,typeof(OfficeSpec).Assembly, typeof(OfficeSpec).Assembly, sut, null, c => c.GetHypermediaConfiguration().EntryPoint.ControllerType = typeof(TestEntryPointController), initializeHelpPage: false);
